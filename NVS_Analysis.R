...
# Scale nutrient quantities for dried foods consumed rehydrated
scale_nutrients <- function(d, nutrients) {
    rows <- which(d$Dried_consumed_rehydrated == "Yes")
    
    if (length(rows) > 0) {
        scale_factors <- d$Energy_kcal[rows] /
            d$Energy_kcal_fresh[rows]
        
        if (any(!is.finite(scale_factors) | scale_factors <= 0)) {
            stop("Invalid energy values for rehydration scaling.")
        }
        
        d[rows, nutrients] <- sweep(
            d[rows, nutrients, drop = FALSE],
            1,
            scale_factors,
            FUN = "/"
        )
...
# Adjust to per 231 g
mineral_cols <- c(
    "Iron_mg", "Zinc_mg", "Calcium_mg",
    "Potassium_mg", "Magnesium_mg"
)

m_M[, mineral_cols] <- sweep(
    m_M[, mineral_cols], 1, 2.31, FUN = "*"
)
...
write.csv(d, "Manuscript/Data/NVS_14Sept2026.csv", row.names = FALSE)
...
write.csv(d, "Manuscript/Data/NVS_nFU_14Sept2026.csv", row.names = FALSE)